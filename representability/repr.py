...
#investigate the representablity of the 1rdm
norb=4

#build operators
rdmop=[]

#FIXME put in symmetries
for a in range(norb):
    for b in range(a,norb):
        ...
        rdmop.append({"a":a,"b":b,"op":op,"sparse":of.get_sparse_operator(op)})
...
for i in in range(len(rdmop)):

    cov.append(np.zeros)


for i in in range(len(rdmop)):
    val=np.dot(np.conj(x),rdmop[i]["sparse"].dot(x))
